...
echo "> enable.sh - activation du module proxy"
echo "Paramètres :"
...
then
    echo -n "En activant le proxy, l'instance Docker va être redémarrée, êtes-vous d'accord ? [o/N]"
    read VALRESTART
    if [ "$VALRESTART" != "O" ] && [ "$VALRESTART" != "o" ] 
    then
        echo "[INF] Fin de la procédure d'activation du proxy..."
        exit 0
    fi
fi

# 4) On peut enfin procéder aux manipulations...
echo "[INF] Début de l'activation du proxy..."
if [ ! -z "$DAEMON_PROXY_DIR" ] 
then
    sudo cp "$HTTPPROXY_CONF_FILEPATH" "$DAEMON_PROXY_DIR/http-proxy.conf"
fi

if [ ! -z "$DOCKER_CONFIG_DIR" ] 
then
    sudo cp "$PROXY_CONFJSON_FILEPATH" "$DOCKER_CONFIG_DIR/config.json"
fi
sudo systemctl daemon-reload ; sudo systemctl restart docker
echo "[INF] Fin de l'activation du proxy..."
